...
try {
    ...
    process.exit(1); // Exit the application if the serial port can't be opened
}
...
async function run() {
    try {
        ...
        parser.on('data', async function(data) {
            try {
                if (!data) {
                    throw new Error('Received empty data from serial port');
                }
                ...
                console.error('Error parsing JSON or inserting into MongoDB:', error);
            }
        });
